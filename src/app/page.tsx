...
export default function HomePage() {
  return (
    <div style={{ background: '#EFE7D4', minHeight: '100vh' }}>
        ...
        <div className="max-w-7xl mx-auto flex items-center gap-3 flex-wrap justify-center">
          ...
          {breeds.map((breed) => (
            <Link key={breed} href={breed === 'All Breeds' ? '/dogs' : `/dogs?breed=${encodeURIComponent(breed)}`} style={{
              ...
              background: breed === 'All Breeds' ? '#D85A1C' : 'rgba(244,239,229,0.05)',
              color: breed === 'All Breeds' ? 'white' : 'rgba(244,239,229,0.6)',
              border: `1px solid ${breed === 'All Breeds' ? '#D85A1C' : 'rgba(244,239,229,0.1)'}`,
            }}>
              {breed}
            </Link>
          ))}
      ...
      <section className="relative overflow-hidden" style={{ height: 400 }}>
        ...
        <div className="absolute inset-0 flex items-center justify-center" style={{ background: 'rgba(10,10,10,0.68)' }}>
          <div className="text-center px-6">
            ...
            <h2 style={{ ...display, fontSize: 'clamp(40px, 6vw, 68px)', color: '#EFE7D4', marginBottom: 28 }}>
              Find Your Next<br /><span style={{ color: '#D85A1C' }}>Hunting Partner.</span>
            </h2>
      ...
      <section style={{ background: '#EFE7D4', padding: '80px 24px' }}>
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-14">
            <div style={{ ...SC, fontSize: 10, color: '#D85A1C', marginBottom: 10 }}>Why GunDog Exchange</div>
            <h2 style={{ ...display, fontSize: 36, color: '#0F0F0E', marginBottom: 8 }}>Built for Hunters</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {features.map((f) => (
              <div key={f.title} style={{ background: 'white', border: '1px solid #D9C8A6', padding: '36px' }}>
                <div style={{ width: 48, height: 48, background: '#0F0F0E', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 20 }}>{f.icon}</div>
                <h3 style={{ ...sans, fontSize: 14, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '-0.01em', color: '#0F0F0E', marginBottom: 10 }}>{f.title}</h3>
      ...
      {/* ── How It Works ─────────────────────────────────── */}
      <section id="how-it-works" style={{ background: '#0F0F0E', padding: '80px 24px' }}>
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-14">
            <div style={{ ...SC, fontSize: 10, color: '#D85A1C', marginBottom: 10 }}>How It Works</div>
            <h2 style={{ ...display, fontSize: 36, color: '#EFE7D4' }}>
              Simple. Safe. Built for<br /><span style={{ color: '#D85A1C' }}>the Field.</span>
            </h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <div key={step.num} style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(244,239,229,0.08)', padding: '36px' }}>
                ...
                <h3 style={{ ...sans, fontSize: 14, fontWeight: 800, textTransform: 'uppercase', color: '#EFE7D4', marginBottom: 10 }}>{step.title}</h3>
