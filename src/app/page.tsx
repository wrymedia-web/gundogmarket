  ...
  return (
    <div style={{ background: '#F4EFE5', minHeight: '100vh' }}>
      ...
      <section
        ...
        style={{ background: '#0E0E0E', minHeight: '620px' }}
      >
        {/* Subtle texture overlay */}
        <div
          className="absolute inset-0"
          style={{
            background: 'radial-gradient(ellipse at 70% 50%, rgba(212,96,10,0.08) 0%, transparent 60%)',
          }}
        />
        <div className="max-w-5xl mx-auto text-center relative z-10">
          ...
          {/* Stats bar */}
          <div
            className="mt-16 inline-flex items-center gap-8 px-8 py-4"
            style={{ border: '1px solid rgba(244,239,229,0.1)', background: 'rgba(255,255,255,0.04)' }}
          >
            {['2,400+ Listings', 'Verified Sellers', 'Secure Escrow'].map((stat, i) => (
              <div key={stat} className="flex items-center gap-4">
                {i > 0 && (
                  <div style={{ width: 1, height: 20, background: 'rgba(244,239,229,0.15)' }} />
                )}
                <span
                  className="text-sm font-semibold uppercase tracking-widest"
                  style={{ color: 'rgba(244,239,229,0.55)', ...inter, letterSpacing: '0.08em' }}
                >
                  {stat}
                </span>
              </div>
      ...
      <section className="px-6 py-24" style={{ background: '#F4EFE5' }}>
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-16">
            ...
            >
              Not just another classifieds site. GunDog Market is purpose-built for the working dog community.
            </p>
          </div>
      ...
      <section id="how-it-works" className="px-6 py-24" style={{ background: '#0E0E0E' }}>
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-16">
            <div
              className="inline-block text-xs font-bold px-4 py-1.5 mb-4 uppercase tracking-widest"
            ...
            {[
              {
                num: '01',
                title: 'Browse & Filter',
                desc: 'Search by breed, training level, location, and price range. View hunt titles, health certs, and seller ratings.',
              },
              {
                num: '02',
                title: 'Contact the Seller',
                desc: 'Message verified sellers directly. Ask for videos, vet records, or a live demo. No middlemen.',
              },
              {
                num: '03',
                title: 'Secure the Deal',
                desc: 'Pay securely through escrow. Funds release only after you receive the dog. Full buyer protection.',
              },
            ].map((step) => (
              <div
                key={step.num}
                className="p-8"
                style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(244,239,229,0.08)' }}
              >
                <div
                  className="text-5xl font-black leading-none mb-4"
                  style={{ ...montserrat, color: '#D4600A', fontWeight: 900 }}
                >
                  {step.num}
                </div>
                <h3
                  className="text-lg font-black uppercase mb-3"
                  style={{ ...montserrat, color: '#F4EFE5' }}
                >
                  {step.title}
                </h3>
                <p
                  className="text-sm leading-relaxed"
                  style={{ color: 'rgba(244,239,229,0.55)', ...inter }}
                >
                  {step.desc}
                </p>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-3xl mx-auto">
            {[
              ...
              >
                {plan.highlight && (
                  <div
                    className="text-xs font-bold px-3 py-1 mb-6 self-start uppercase tracking-widest"
                    style={{ background: '#D4600A', color: 'white', ...inter }}
                  >
                    Most Popular
                  </div>
                ...
                <div
                  className="mb-1 font-black text-xl uppercase"
                  style={{ ...montserrat, color: plan.highlight ? '#F4EFE5' : '#0E0E0E' }}
                >
                  {plan.name}
                </div>
                <div className="flex items-baseline gap-1 mb-6 mt-2">
                  ...
                  <span
                    className="text-sm"
                    style={{ color: plan.highlight ? 'rgba(244,239,229,0.5)' : '#7C7A6E', ...inter }}
                  >
                    {plan.desc}
                  </span>
                </div>
                <ul className="space-y-3 mb-8 flex-1">
                  {plan.features.map((feat) => (
                    <li
                      ...
                      style={{ color: plan.highlight ? 'rgba(244,239,229,0.75)' : '#7C7A6E', ...inter }}
                    >
                ...
                <Link
                  href="/sell"
                  className="block text-center text-sm font-bold py-3.5 uppercase tracking-widest transition-colors"
                  style={{
      ...
      >
        <div className="flex items-center justify-center gap-1 mb-4">
          ...
          <span
            className="text-lg font-black"
            style={{ ...montserrat, color: 'rgba(244,239,229,0.4)', letterSpacing: '-0.02em' }}
          >
            MARKET
          </span>
        </div>
        ...
        <p
          className="text-xs"
          style={{ color: 'rgba(255,255,255,0.2)', ...inter }}
        >
          © 2026 GunDog Market. The Marketplace for Working Dogs.
        </p>
      </footer>
    </div>
  )
